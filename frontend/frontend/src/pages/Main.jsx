...
function Main() {
    ...
    useEffect(() => {
        if (submitButtonClicked) {
            if(r_val<0){
                window.alert("choose the correct radius")
                return;
            }
    ...
    function getStyle(point){
        if(point.hit && point===points[points.length-1]){
            return styles.latest_green;
        }else if(!point.hit && point===points[points.length-1]){
            return styles.latest_red;
        } else return styles.not_latest;
    }
        ...
        <div className={styles.container}>
            ...
            <div className={styles.tableContainer}>
                <table className={styles.table}>
                    ...
                    <tbody>
                    { points.map((pt) => (
                        <tr className={getStyle(pt)} key={pt.id}>
                            <td>{pt.x}</td>
                            <td>{pt.y}</td>
                            <td>{pt.r}</td>
